...
var scenePilot = function () {
    ...
    obj.dataTableInit = function () {
        var option = {
            ele: $('#dataTable'),
            ajax: {url: getScenePilotListUrl + "?type=1&name=" + $("#txtQuery").val(), type: "POST"},
            columns: [
...
// 查询
    obj.evtOnQuery = function () {
        dataTable.ajax.url(getScenePilotListUrl + "?type=1&name=" + encodeURIComponent($("#txtQuery").val()));
...
// 刷新
    obj.evtOnRefresh = function () {
        $("#txtQuery").val("");
        dataTable.ajax.url(getScenePilotListUrl);
        dataTable.ajax.reload();
    }
...
// 保存用户
    obj.evtOnSave = function (index, id) {
        ...
        setTimeout(function () {
            ...
            oData["pilotType"] = 1;//1是全页面导航.2是场景导航
            oData["pilotUrl"]=$("#pilotUrl").val();
...
// 重置内容
    ...
    obj.evtOnShowAddition=function(){
        //layerIndex = $html.iProbeBox(650, $('#popupAddition'), obj.resetRules);
        //layerIndex = $html.iProbeBox(800, 500, "限制条件", $('#popupAddition'), obj.resetRules);
        layer.open({
            type: 1,
            title: "限制条件",
            closeBtn: 0,
...
// 新增规则
    obj.evtOnAddAddition=function(id) {
        ...
        var operatorValue = $('#rulesData' + id + ' select').val();
...
// 新增规则 运算符
    ...
    obj.evtOnHelp=function() {
        ...
        //layerHelp = $html.iProbeBox(500, $('#popupHelp'), null);
        //$("#popupHelp").parent().css("position","initial")
    }
