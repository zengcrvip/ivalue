    ...
    $.fn.tagEditor = function (options, val, blur) {
        ...
        if (typeof options == 'string') {
            ...
            selector.each(function () {
                ...
                if (options == 'getTags')
                    ...
                    // insert new tag
                    var li = $('<li><div class="tag-editor-spacer">&nbsp;' + o.delimiter[0] + '</div><div class="tag-editor-tag"></div><div class="tag-editor-delete"><i></i></div></li>').appendTo(ed).find('.tag-editor-tag');

                    li.html('<input type="text" maxlength="' + o.maxLength + '">').addClass('active');

                    var input = li.find('input').val(val);

                    input.blur();

                    if (val == "(" || val == ")" || val == "+" || val == "-" || val == "*" || val == "/") {
                        $(".tag-editor-tag:contains('" + val + "')").attr("data-rules", val);
                    }
                    else if (val == "||" || val == "&&") {
                        $(".tag-editor-tag:contains('" + dictionaryMatch(val) + "')").attr("data-rules", val);
                    }
        ...
        if (window.getSelection) $(document).off('keydown.tag-editor').on('keydown.tag-editor', function (e) {
            if (e.which == 8 || e.which == 46 || e.ctrlKey && e.which == 88) {
                try {
                    ...
                    var el = document.activeElement.tagName == 'BODY' ? $(sel.getRangeAt(0).startContainer.parentNode).closest('.tag-editor') : 0;
                } catch (e) {
                    el = 0;
                }
                ...
                if (sel.rangeCount > 0 && el && el.length) {
                    var tags = []
                    var splits = sel.toString().split(el.prev().data('options').dregex);
        ...
        return selector.each(function () {
            ...
            ed.on('click', '.tag-editor-delete', function (e) {
                ...
                tag.addClass('deleted').animate({ width: 0 }, o.animateDelete,
                    function () {
            ...
            if (o.clickDelete)
                ed.on('mousedown', '.tag-editor-tag', function (e) {
                    if (e.ctrlKey || e.which > 1) {
                        var li = $(this).closest('li'), tag = li.find('.tag-editor-tag'), prevLi = $(this).closest('li').prev();
                        if (o.beforeTagDelete(el, ed, tag_list, tag.text()) === false) return false;

                        tag.addClass('deleted').animate({ width: 0 }, o.animateDelete,
                            function () {
                                li.remove();
                                if (prevLi.html() == "&nbsp;") prevLi.remove();
                                set_placeholder();
                            });
                        update_globals();
            ...
            function split_cleanup(input) {
                var li = input.closest('li'),
                    ...
                    //sub_tags = convertToStrings(input.val().trim()),
            ...
            ed.on('blur', 'input', function (e) {
                ...
                // 自定义添加部分
                var alphaTag = dictionaryMatchByName(tag)
                var engFirstTag = ""
                if (isVerifiedTag(alphaTag)) {
                    editDiv.attr("data-rules", alphaTag).html(escape(dictionaryMatch(alphaTag))).removeClass('active');

                } else {
                    var cloneTag = alphaTag;
                    var isGenerateTagTillTail = true  // 如果尾部的字符串是合法的tag，是否也生成tag

                    var firstTag = getFirstVerifiedTag(cloneTag, isGenerateTagTillTail);
                    if (firstTag[0] != "") {

                        engFirstTag = dictionaryMatchByName(firstTag[0]);
                        editDiv.html(escape(dictionaryMatch(engFirstTag))).attr("data-rules", "" + engFirstTag + "").removeClass('active');

                        cloneTag = firstTag[1];
                        firstTag = getFirstVerifiedTag(cloneTag, isGenerateTagTillTail);
                    }
                    while (firstTag[0] != "") {

                        engFirstTag = dictionaryMatchByName(firstTag[0]);

                        var li = $('<li style="width:1px">&nbsp;</li><li class="' + replaceColor(engFirstTag) + '"><div class="tag-editor-spacer">&nbsp;,</div><div class="tag-editor-tag" data-rules="' + engFirstTag + '">' + escape(dictionaryMatch(engFirstTag)) + '</div><div class="tag-editor-delete"><i></i></div></li>')

                        if (currentLi && currentLi.length > 0) {
                            currentLi.closest('li').after(li).removeClass('active');
                            currentLi = currentLi.next().next();
                        } else {
                            currentLi = editDiv.parent().closest('li').after(li).removeClass('active');
                            currentLi = currentLi.next().next();

                            //currentLi = $(currentLi.siblings('li')[currentLi.siblings('li').length - 1]);
                        }


                        //$(".tag-editor").append(li).removeClass('active');

                        //input.parent().attr("data-rules", firstTag[0]).html(escape(dictionaryMatch(firstTag[0]))).removeClass('active');

                        cloneTag = firstTag[1];
                        firstTag = getFirstVerifiedTag(cloneTag, isGenerateTagTillTail);
                    }

                    if (cloneTag != "" && firstTag[0] != "") {

                        engFirstTag = dictionaryMatchByName(firstTag[0]);

                        var li = $('<li style="width:1px">&nbsp;</li><li class="' + replaceColor(engFirstTag) + '"><div class="tag-editor-spacer">&nbsp;,</div><div class="tag-editor-tag" data-rules="' + engFirstTag + '">' + escape(dictionaryMatch(cloneTag)) + '</div><div class="tag-editor-delete"><i></i></div></li>')

                        editDiv.parent().closest('li').after(li).addClass('active');

                        //$(".tag-editor").append(li).addClass('active');

                        //input.parent().attr("data-rules", cloneTag).html(escape(dictionaryMatch(cloneTag))).removeClass('active');
                    }
                }
            ...
            ed.on('keydown', 'input', function (e) {
                ...
                if ((e.which == 37 || !o.autocomplete && e.which == 38) && !$t.caret() || e.which == 8 && !$t.val()) {
                    var prev_tag = [];
                    var prev_li = $t.closest('li').prev('li');
                    var prev_prev_li = $t.closest('li').prev('li').prev('li');
                    if (prev_li.find('.tag-editor-tag')[0] != undefined) {
                        prev_tag = prev_li.find('.tag-editor-tag')
                    } else if (prev_li.find('.tag-editor-tag')[0] == undefined && prev_prev_li[0] != undefined) {
                        prev_li.remove();
                        if (prev_prev_li.find('.tag-editor-tag')[0] != undefined) {
                            prev_tag = prev_prev_li.find('.tag-editor-tag')
                        }
                    }
                    if (prev_tag.length) {
                    ...
                    if ($t.val().length == 0) {
                        var prevEle = $t.parent().parent().prev();

                        if (prevEle.children().length <= 0) {

                            var ele = prevEle.prev().children("div.tag-editor-tag");

                            ele.append($("<input type='text' maxlength='50' value=" + $t.val() + " style='width: 35px;'>        <span style='position: absolute; top: -9999px; left: -9999px; width: auto; font-size: 16px; font-family: 微软雅黑, 宋体, Arial, Helvetica, Verdana, sans-serif; font-weight: 400; letter-spacing: 0px; white-space: nowrap;'>" + $t.val() + "</span>"));
                        }
                    }
                }
            });
            ...
            for (var i = 0; i < tags.length; i++) {
                ...
                if (tag) {
                    ...
                    ed.append('<li class="' + className + '"><div class="tag-editor-spacer">&nbsp;' + o.delimiter[0] + '</div><div class="tag-editor-tag" data-rules="' + escape(dictionaryMatchByName(tag)) + '">' + escape(tag) + '</div><div class="tag-editor-delete"><i></i></div></li>');
                }
            }
            //}
