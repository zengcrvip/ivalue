...
var ZYFILE = {
    ...
    funDeleteFile: function (delFileIndex, isCb) {
        ...
        var $cloneFileInput = $('<input  id="fileImage" type="file" size="30" name="fileselect[]" style="display: none;">');
        var $parentBtn = $($fileInput).parent();
        $fileInput.remove();
        $parentBtn.append($cloneFileInput);
        $cloneFileInput[0].addEventListener("change", function (e) {
            self.funGetFiles(e);
        })
...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
            ...
            this.createHtml = function () {
                ...
                if (para.dragDrop) {
                    ...
                    if (para.del) {  // 显示删除按钮
                        delHtml = '<span class="file_del" data-index="0" title="删除"></span>';
                    }
                    // 创建不带有拖动的html
                    html += '<form id="uploadForm" action="' + para.url + '" method="post" enctype="multipart/form-data">';
                    html += '	<div class="upload_box">';
                    ...
                    html += '<div id="uploadList_1' + '" class="upload_append_list">';
                    html += '	<div class="file_bar">';
                    html += '		<div style="padding:5px;">';
                    html += '			<p class="file_name" title=""></p>';
                    ...
                    html += '			<img id="uploadImage_0" class="upload_file" src="ext/zyupload/skins/images/add_img.png" style="width:expression(this.width > ' + imgWidth + ' ? ' + imgWidth + 'px : this.width)" />';
                    html += '		</div>';
                    html += '	</a>';
                    html += '	<p id="uploadProgress_1" class="file_progress"></p>';
                    html += '	<p id="uploadFailure_1" class="file_failure">上传失败，请重试</p>';
                    html += '	<p id="uploadSuccess_1" class="file_success"></p>';
                    html += '</div>';
                ...
                for (var i = 0, file; file = files[i]; i++) {
                    ...
                    if (newStr.split(".")[0] != null) {
                        ...
                        if (jQuery.inArray(type, para.fileType) > -1) {
                            ...
                            alert('您这个"' + file.name + '"上传类型不正确,请重新上传！');
                        }
                    } else {
            ...
            this.funDisposePreviewHtml = function (file, e) {
                ...
                var type = newStr.split(".")[0].split("").reverse().join("");
                // 处理不同类型文件代表的图标
                var fileImgSrc = "ext/zyupload/skins/images/fileType/";
                if (type == "rar") {
                ...
                if (file.type.indexOf("image") == 0) {
                    ...
                    html += '			<img id="uploadImage_0" class="upload_image" src="' + e.target.result + '" style="width:expression(this.width > ' + imgWidth + ' ? ' + imgWidth + 'px : this.width);" />';
                    html += '		</div>';
                    html += '	</a>';
                    html += '	<p id="uploadProgress_1" class="file_progress"></p>';
                    html += '	<p id="uploadFailure_1" class="file_failure">上传失败，请重试</p>';
                    html += '	<p id="uploadTailor_1" class="file_tailor" tailor="false">裁剪完成</p>';
                    html += '	<p id="uploadSuccess_1" class="file_success"></p>';
                    html += '</div>';
                    ...
                    html += '	<a style="height:' + para.itemHeight + ';width:' + para.itemWidth + ';" href="#" class="imgBox">';
                    html += '		<div class="uploadImg" style="width:' + imgWidth + 'px">';
                    html += '			<img id="uploadImage_0" class="upload_file" src="' + fileImgSrc + '" style="width:expression(this.width > ' + imgWidth + ' ? ' + imgWidth + 'px : this.width)" />';
                    html += '		</div>';
                    html += '	</a>';
                    html += '	<p id="uploadProgress_1" class="file_progress"></p>';
                    html += '	<p id="uploadFailure_1" class="file_failure">上传失败，请重试</p>';
...
//								}
                    ...
                    onDelete: function (file, files) {
                        ...
                        $(".file_bar")[0].parentNode.removeChild($(".file_bar")[0]);
                    ...
                    onSuccess: function (file, response) {
                        ...
                        $("#uploadProgress_1").hide();
                        $("#uploadSuccess_1").show();
                        //$("#uploadInf").append("<p>上传成功，文件地址是：" + response + "</p>");
                        // 根据配置参数确定隐不隐藏上传成功的文件
                        if (para.finishDel) {
                            // 移除效果
                            $("#uploadList_1").fadeOut();
                            // 重新设置统计栏信息
                            self.funSetStatusInfo(ZYFILE.funReturnNeedFiles());
                        }
                        if ($("#uploadTailor_1").length > 0) {
                            $("#uploadTailor_1").hide();
                        }
                    },
                    onFailure: function (file, response) {
