 ...
 */
@Service("nightPerTaskExecutor")
public class NightPerTaskExecutor implements TaskExecutor
{

    private static final Logger LOG = Logger.getLogger(NightPerTaskExecutor.class.getName());
    ...
    {
        Date date = new Date();
        LOG.info("Shop Night Busy Task Executor : " + domain.getTaskName() + " " + date.toString());

        Integer taskId = domain.getSmsTaskId();
        Integer marketUserCountLimit = (domain.getMarketLimit() == null || domain.getMarketLimit() >  1000) ? 1000 : domain.getMarketLimit();
        ...
        if (baseAreaId != null && baseId != null)
        {
            ...
            sendShopSmsService.sendSms(sendShopSmsService.createSelectDataFromModelSql(fullTableName, null, baseId,baseAreaId, 100), 100, marketUserCountLimit, smsMessage, taskId, accessNumber,baseId,2,1);
        }
        return null;
    }
}
