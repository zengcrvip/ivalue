...
public class ExportUtils
{
    ...
    private static ExportUtils excelUtil = null;
            ...
            for (int colidx = 0; colidx < colList.size(); colidx++)
            {
                ...
                if (rowSpan * colSpan > 1)
                {
                    ...
                    for (int kk = readCol + 1; kk <= readCol + colSpan - 1; kk++)
                    {
                        ...
                        cellMerge.setCellStyle(nvlStyle(newStyleMap, styleMap, ExportUtils.SKEY_MERGE));
                    }
                ...
                switch (rowType)
                {
                    case ExcelRowEntity.TITLE_ROW://大标题行
                        cell.setCellStyle(nvlStyle(newStyleMap, styleMap, ExportUtils.SKEY_TITLE));
                        break;
                    case ExcelRowEntity.SMALL_TITLE_ROW://小标题行
                        cell.setCellStyle(nvlStyle(newStyleMap, styleMap, ExportUtils.SKEY_SMALL_TITLE));
                        break;
                    case ExcelRowEntity.HEAD_ROW://表头行
                        cell.setCellStyle(nvlStyle(newStyleMap, styleMap, ExportUtils.SKEY_HEAD));
                    ...
                    case ExcelRowEntity.SPLIT_ROW://分割行
                        cell.setCellStyle(nvlStyle(newStyleMap, styleMap, ExportUtils.SKEY_ROW_SPLIT));
                        break;
                    default://默认是数据行
                        cell.setCellStyle(nvlStyle(newStyleMap, styleMap, ExportUtils.SKEY_DATA));
                }
            }
        }
        try
    ...
    private Map<String, HSSFCellStyle> initDefaultStyleMap(HSSFWorkbook workbook)
    {
        ...
        style.setFont(font);
        styleMap.put(ExportUtils.SKEY_TITLE, style);
        ...
        styleMap.put(ExportUtils.SKEY_SMALL_TITLE, style1);
        ...
        styleMap.put(ExportUtils.SKEY_HEAD, style2);
        ...
        styleMap.put(ExportUtils.SKEY_DATA, style3);
        ...
        styleMap.put(ExportUtils.SKEY_ROW_SPLIT, style5);
        ...
        styleMap.put(ExportUtils.SKEY_MERGE, style6);
    ...
    private int getStrLenth(String str)
    {
        ...
        len = len < ExportUtils.MIN_COLUMN_WIDTH ? ExportUtils.MIN_COLUMN_WIDTH : len;
        len = len > ExportUtils.MAX_COLUMN_WIDTH ? ExportUtils.MAX_COLUMN_WIDTH : len;
        return len;
    }
